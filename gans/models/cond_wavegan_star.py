...
# from cond_batchnorm import ConditionalBatchNorm1d
...
class CondWaveGANGenerator(nn.Module):
       ...
       # self.deconv_4 = Transpose1dLayer( model_size, model_size, 25, stride, upsample=upsample)
        ...
        for m in self.modules():
            if isinstance(m, nn.ConvTranspose1d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight.data)    #改过
    ...
    def forward(self, x, y):
        ...
        #这里原来是32，5*self.model_size, -1
        x = x.view(x.size(0), 5*self.model_size, -1)        #x = self.fc1(x).view(-1, 16 * self.model_size, 16)
        x = F.relu(x)
